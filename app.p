...
class User(db.Model):
    ...
    status_type = db.Column(db.String(10), default='text') # text, image, video
...
def chat():
    ...
    now = get_waktu_wita()
    my_story = {
        'content': user.status_note if (user.note_expires_at and user.note_expires_at > now) else "",
        'type': user.status_type if (user.note_expires_at and user.note_expires_at > now) else "text"
    }
    ...
    for relasi in Contact.query.filter_by(user_id=user.id).all():
        ...
        if teman:
            ...
            daftar_teman.append({
                ...
                'story_type': teman.status_type if has_story else "text"
            })
    ...
    return render_template('index.html', user_aktif=user, my_story=my_story, daftar_teman=daftar_teman, daftar_grup=daftar_grup)
